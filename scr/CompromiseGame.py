...
class HumanPlayer(AbstractPlayer):
    ...
    def printNumber(self, k, i, j, n, color):
        if n == 0:
            if self.order == 0:
                ...
                s = ". "
            self.screen.addstr(
                k + 3, 25 * i + 6 * j + 8 + 2 * self.order, s, self.colorN
            )
            return
        if n < 10:
            s = " " + str(n)
    ...
    def setParams(
        self, screenref, colourDefault, colourHighlight, colourNeutral, playOrder
    ):
    ...
    def placePips(self, myState, oppState, myScore, oppScore, turn, length, nPips):
        ...
        while True:
            ...
            if key == 114 or key == 82:
                ...
                self.flushPlacements()
                self.screen.addstr(
                    9, 50, "Need " + str(nPips) + ", placed 0  ", self.color
                )
                self.screen.addstr(10, 50, "Place your pips        ", self.color)
                self.screen.addstr(11, 50, "                       ", self.color)
                for k in range(3):
                ...
                if placed < nPips:
                    try:
                        ...
                        self.printNumber(
                            ...
                            i,
                            j,
                            myState[i][k][j] + self.placements[k][i][j],
                            self.colorH,
                        )
                        self.screen.addstr(
                            9,
                            50,
                            "Need " + str(nPips) + ", placed " + str(placed),
                            self.color,
                        )
                        if placed == nPips:
                            self.screen.addstr(
...
class CompromiseGame:
    ...
    def roundStart(self):
        ...
        if self.type == "s":
            for i in range(0, self.newPips):
                self.redPips[random.randint(0, 2)][random.randint(0, 2)][
                    random.randint(0, 2)
                ] += 1
                self.greenPips[random.randint(0, 2)][random.randint(0, 2)][
                    random.randint(0, 2)
                ] += 1
        else:
            self.prepareDisposable()
            greenplacing = self.greenPlayer.placePips(
                self.greenDisposable,
                self.redDisposable,
                self.greenScore,
                self.redScore,
            ...
            self.prepareDisposable()
            redplacing = self.redPlayer.placePips(
                self.redDisposable,
                self.greenDisposable,
                self.redScore,
                self.greenScore,
    ...
    def getMoves(self):
        if self.type == "g":
            ...
            self.prepareDisposable()
            self.redMove = self.redPlayer.play(
                self.redDisposable,
                self.greenDisposable,
                self.redScore,
                self.greenScore,
            ...
            self.prepareDisposable()
            self.greenMove = self.greenPlayer.play(
                self.greenDisposable,
                self.redDisposable,
                self.greenScore,
                self.redScore,
            ...
            if not isinstance(self.greenMove, list):
                raise Exception(
                    "Green Player's move is not a list: " + str(self.greenMove)
                )
            ...
            if not len(self.greenMove) == 3:
                raise Exception(
                    "Green Player's move's length has to be 3: " + str(self.greenMove)
                )
            if not len(self.redMove) == 3:
                raise Exception(
                    "Red Player's move's length has to be 3: " + str(self.redMove)
                )
            for i in range(3):
                ...
                if self.redMove[i] > 2 or self.redMove[i] < 0:
                    raise Exception(
                        "Red Player's move is not between 0 and 2: " + str(self.redMove)
                    )
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                for k in range(0, 3):
                    if not (
                        i == self.redMove[0]
                        or i == self.greenMove[0]
                        or j == self.redMove[1]
                        or j == self.greenMove[1]
                        or k == self.redMove[2]
                        or k == self.greenMove[2]
                    ):
    ...
    def play(self):
        while self.turn < self.gameLength or (
            self.noties and self.redScore == self.greenScore
        ):
            self.playRound()
        ...
        for k in range(3):
            for i in range(3):
                for j in range(3):
                    if not (
                        k == self.redMove[0]
                        or k == self.greenMove[0]
                        or i == self.redMove[1]
                        or i == self.greenMove[1]
                        or j == self.redMove[2]
                        or j == self.greenMove[2]
                    ):
                        if self.redPips[k][i][j] > 0:
                            if self.redPips[k][i][j] < 10:
                                s = " " + str(self.redPips[k][i][j])
    ...
    def fancyPrintMoves(self, stdscr):
        stdscr.addstr(
            8,
            20,
            str(self.redMove[0] + 1)
            + str(self.redMove[1] + 1)
            + str(self.redMove[2] + 1),
            curses.color_pair(1),
        )
        stdscr.addstr(
            9,
            20,
            str(self.greenMove[0] + 1)
            + str(self.greenMove[1] + 1)
            + str(self.greenMove[2] + 1),
            curses.color_pair(2),
        )
